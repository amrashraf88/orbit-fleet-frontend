import { Bell, CaretLeft, SignOut, Sparkle, X } from "@phosphor-icons/react";
import { navigationGroups } from "@/src/config/navigation";
import type { PageId } from "@/src/types/navigation";

interface SidebarProps { page: PageId; onPageChange: (page: PageId) => void; open: boolean; onClose: () => void; onLogout: () => void }

export function Sidebar({ page, onPageChange, open, onClose, onLogout }: SidebarProps) {
  ...
  return <aside className={`side-nav ${open ? "is-open" : ""}`}>
    ...
    <button className="sidebar-logout" onClick={onLogout}><SignOut weight="duotone"/><span>تسجيل الخروج</span></button><button className="assistant-card" onClick={() => notify("المساعد الذكي جاهز للربط بخدمة المحادثة الخاصة بك")}><div className="assistant-icon"><Sparkle weight="fill" /></div><div><b>المساعد الذكي</b><span>تحليلات فورية لأسطولك</span></div><CaretLeft className="assistant-arrow"/></button>
  </aside>;
}
