...
const stateMeta: Record<VehicleState, { tone: string; label: string }> = { moving: { tone: "green", label: "متحركة" }, idle: { tone: "yellow", label: "خاملة" }, stopped: { tone: "pink", label: "متوقفة" }, online: { tone: "cyan", label: "متصلة" }, offline: { tone: "gray", label: "غير متصلة" } };

export function VehicleRow({ vehicle, selected, onClick }: { vehicle: Vehicle; selected: boolean; onClick: () => void }) {
  const { tone, label } = stateMeta[vehicle.state] ?? stateMeta.offline;
